...
function FormulaValueInput({
  ...
  const [valueType, setValueType] = useState<'fixed' | 'formula'>(isFormula ? 'formula' : 'fixed');
      ...
      {valueType === 'fixed' ? (
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            公式中可以引用自身面板和对手面板，例如: self_max_hp * 0.5, opponent_hp * 0.3
          </p>
        </div>
      )}
    </div>
...
export default function ConditionEditor({
  ...
  function getConditionType(cond: Condition): 'cultivation' | 'battle' | 'and' | 'or' {
  ...
  const handleTypeChange = (type: 'none' | 'cultivation' | 'battle' | 'and' | 'or') => {
    ...
    if (type === 'none') {
      ...
      onChange({ attribute_comparison: { attribute: 'comprehension', op: 'greater_than', value: 0 } });
    } else {
      onChange({ self_attribute_comparison: { attribute: 'hp', op: 'greater_than', value: 0 } });
    }
  };
  ...
  const renderCultivationCondition = () => {
        ...
        <div className="space-y-3 p-4 bg-white rounded-lg border-2 border-indigo-200 shadow-sm">
          ...
          <Select
            ...
            options={[
              ...
              { value: 'attack_skill_type_is', label: '修行的攻击武技类型为指定类型' },
              { value: 'defense_skill_is', label: '修行的防御武技为指定武技' },
              { value: 'defense_skill_type_is', label: '修行的防御武技类型为指定类型' },
    ...
    if (conditionKey === 'internal_is') {
      idOptions = internals.map(item => ({ value: item.id, label: item.name }));
    } else if (conditionKey === 'attack_skill_is') {
      idOptions = attackSkills.map(item => ({ value: item.id, label: item.name }));
    } else if (conditionKey === 'defense_skill_is') {
      idOptions = defenseSkills.map(item => ({ value: item.id, label: item.name }));
      ...
      <div className="space-y-3 p-4 bg-white rounded-lg border-2 border-indigo-200 shadow-sm">
        ...
        {conditionKey !== 'attribute_comparison' && (
          loading ? (
            ...
            <SearchableSelect
              label={conditionKey === 'internal_is' ? '内功' : 
                     conditionKey === 'attack_skill_is' ? '攻击武技' :
                     conditionKey === 'defense_skill_is' ? '防御武技' :
                     '特性'}
              options={idOptions.length > 0 ? idOptions : [{ value: '', label: '暂无数据' }]}
              value={conditionValue || ''}
              onChange={(selectedValue) =>
                onChange({ [conditionKey]: selectedValue } as any)
              }
              placeholder={`搜索${conditionKey === 'internal_is' ? '内功' : 
                     conditionKey === 'attack_skill_is' ? '攻击武技' :
                     conditionKey === 'defense_skill_is' ? '防御武技' :
                     '特性'}...`}
            />
        ...
        <div className="space-y-3 p-4 bg-white rounded-lg border-2 border-orange-200 shadow-sm">
          ...
          <Select
            ...
            options={[
              ...
              { value: 'opponent_attack_skill_is', label: '对手攻击武技为指定武技' },
              { value: 'opponent_defense_skill_is', label: '对手防御武技为指定武技' },
              { value: 'opponent_internal_type_is', label: '对手内功类型为指定类型' },
              { value: 'opponent_attack_skill_type_is', label: '对手攻击武技类型为指定类型' },
              { value: 'opponent_defense_skill_type_is', label: '对手防御武技类型为指定类型' },
              { value: 'attack_broke_qi_defense', label: '攻击击破敌方内息防御' },
              { value: 'attack_did_not_break_qi_defense', label: '攻击未击破敌方内息防御' },
              { value: 'successfully_defended_with_qi', label: '成功内息防御敌方攻击' },
              { value: 'failed_to_defend_with_qi', label: '未成功内息防御敌方攻击' },
            ]}
            ...
            onChange={(e) => {
              ...
              if (key === 'self_attribute_comparison') {
                ...
                onChange({ opponent_attribute_comparison: { attribute: 'hp', op: 'greater_than', value: 0 } });
      ...
      <div className="space-y-3 p-4 bg-white rounded-lg border-2 border-orange-200 shadow-sm">
        ...
        {conditionKey.includes('_is') && (
          loading ? (
            ...
            <SearchableSelect
              label={conditionKey === 'opponent_internal_is' ? '内功' : 
                     conditionKey === 'opponent_attack_skill_is' ? '攻击武技' :
                     '防御武技'}
              options={idOptions.length > 0 ? idOptions : [{ value: '', label: '暂无数据' }]}
              value={conditionValue || ''}
              onChange={(selectedValue) =>
              ...
              placeholder={`搜索${conditionKey === 'opponent_internal_is' ? '内功' : 
                     conditionKey === 'opponent_attack_skill_is' ? '攻击武技' :
                     '防御武技'}...`}
            />
          ) : (
  ...
  const renderAndOrCondition = () => {
    if (!condition || (conditionType !== 'and' && conditionType !== 'or')) return null;

    const conditions = conditionType === 'and' ? (condition as any).and : (condition as any).or;
    ...
    return (
      <div className="space-y-3 p-4 bg-white rounded-lg border-2 border-teal-200 shadow-sm">
        ...
        <Button
          size="sm"
          onClick={() => {
            const newConditions = [...conditions, { attribute_comparison: { attribute: 'comprehension', op: 'greater_than', value: 0 } }];
            onChange({ [conditionType]: newConditions } as any);
          }}
        >
          添加子条件
