...
function formatFormulaValueText(value: FormulaValue): {
  text: string;
  isFormula: boolean;
  hasVars: boolean;
} {
...
function describeEffect(effect: Effect): string {
  ...
  const panelLabel =
    PANEL_TARGET_LABELS[effect.target_panel ?? "own"] ?? "自身";
  ...
  const isPercentUnit =
    ...
    PERCENT_LIKE_TARGETS.has(effect.target);
  const valueText =
    isPercentUnit && !valueMeta.isFormula
