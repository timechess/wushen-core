...
export function isConditionMet(
  ...
  if ('and' in condition) {
    return condition.and.every((item) => isConditionMet(item, character, manuals));
  }
  if ('or' in condition) {
    return condition.or.some((item) => isConditionMet(item, character, manuals));
  }
  ...
  const internalType = internalId ? manuals.internals[internalId]?.manual_type ?? null : null;
  const attackType = attackId ? manuals.attackSkills[attackId]?.manual_type ?? null : null;
  const defenseType = defenseId ? manuals.defenseSkills[defenseId]?.manual_type ?? null : null;
