...
export default function NewDefenseSkillPage() {
  const router = useRouter();
  const { activePack } = useActivePack();
  const [skill, setSkill] = useState<DefenseSkill>({
    id: '',
    name: '',
    description: '',
    rarity: 1,
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      const defaultRealms: DefenseSkillRealm[] = Array.from({ length: 5 }, (_, i) => ({
        level: i + 1,
        exp_required: 0,
  ...
  return (
    <RequireActivePack title="创建防御武技前需要先选择一个模组包。">
      <div className="page-shell">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">新建防御武技</h1>
              <p className="text-gray-600">创建新的防御武技并配置境界</p>
            </div>
