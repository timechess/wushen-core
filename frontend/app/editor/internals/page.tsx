...
export default function InternalsEditorPage() {
  ...
  const visibleInternals = useMemo(() => {
    const normalized = searchQuery.trim().toLowerCase();
    if (!normalized) return internals;
    return internals.filter((internal) => internal.name.toLowerCase().startsWith(normalized));
  }, [internals, searchQuery]);
              ...
              <>
                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/80">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="text-sm text-gray-600">共 {visibleInternals.length} 条</div>
                ...
                {visibleInternals.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">没有匹配的内功</div>
                ) : (
