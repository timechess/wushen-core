            ...
            {EDITOR_LINKS.map((item) => (
              <a
                ...
                className={`flex items-center justify-between rounded-lg border px-5 py-4 transition-colors ${
                  activePack
                    ? "border-[var(--app-border)] hover:border-[var(--app-accent)] hover:bg-[var(--app-accent-soft)]"
                    : "border-[var(--app-border)] bg-[var(--app-surface-muted)] text-gray-400 cursor-not-allowed"
                }`}
                onClick={(event) => {
                  if (!activePack) {
