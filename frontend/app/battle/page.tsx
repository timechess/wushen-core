...
export default function BattlePage() {
  ...
  const [attackerQiOutputRate, setAttackerQiOutputRate] = useState<
    number | undefined
  >(undefined);
  const [defenderQiOutputRate, setDefenderQiOutputRate] = useState<
    number | undefined
  >(undefined);
  ...
  const currentAttackerPanel =
    battleCurrentPanels?.attacker ?? battleResult?.attacker_panel ?? null;
  const currentDefenderPanel =
    battleCurrentPanels?.defender ?? battleResult?.defender_panel ?? null;
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setAttackerQiOutputRate(
                      value === "" ? undefined : parseFloat(value) / 100,
                    );
                  }}
                />
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
                ...
                {currentAttackerPanel && (
                  <div className="text-base text-gray-700 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 pt-2 border-t border-gray-200">
                      <div>
                        基础攻击 {currentAttackerPanel.base_attack.toFixed(1)}
                      </div>
                      <div>
                        基础防御 {currentAttackerPanel.base_defense.toFixed(1)}
                      </div>
                      <div>
                        内息输出{" "}
                        {(currentAttackerPanel.qi_output_rate * 100).toFixed(1)}
                        %
                      </div>
                      <div>
                        内息质量 {currentAttackerPanel.qi_quality.toFixed(2)}
                      </div>
                      <div>
                        增伤{" "}
                        {(currentAttackerPanel.damage_bonus * 100).toFixed(1)}%
                      </div>
                      <div>
                        减伤{" "}
                        {(currentAttackerPanel.damage_reduction * 100).toFixed(
                          1,
                        )}
                        %
                      </div>
                      ...
                      <div>
                        出手速度 {currentAttackerPanel.attack_speed.toFixed(2)}
                      </div>
                      <div>
                        回气率{" "}
                        {(currentAttackerPanel.qi_recovery_rate * 100).toFixed(
                          1,
                        )}
                        %
                      </div>
                      <div>
                        蓄力时间 {currentAttackerPanel.charge_time.toFixed(2)}
                      </div>
                    </div>
                  </div>
                )}
              ...
              <section className="surface-panel p-4 flex flex-col gap-3">
                ...
                {currentDefenderPanel && (
                  <div className="text-base text-gray-700 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 pt-2 border-t border-gray-200">
                      <div>
                        基础攻击 {currentDefenderPanel.base_attack.toFixed(1)}
                      </div>
                      <div>
                        基础防御 {currentDefenderPanel.base_defense.toFixed(1)}
                      </div>
                      <div>
                        内息输出{" "}
                        {(currentDefenderPanel.qi_output_rate * 100).toFixed(1)}
                        %
                      </div>
                      <div>
                        内息质量 {currentDefenderPanel.qi_quality.toFixed(2)}
                      </div>
                      <div>
                        增伤{" "}
                        {(currentDefenderPanel.damage_bonus * 100).toFixed(1)}%
                      </div>
                      <div>
                        减伤{" "}
                        {(currentDefenderPanel.damage_reduction * 100).toFixed(
                          1,
                        )}
                        %
                      </div>
                      ...
                      <div>
                        出手速度 {currentDefenderPanel.attack_speed.toFixed(2)}
                      </div>
                      <div>
                        回气率{" "}
                        {(currentDefenderPanel.qi_recovery_rate * 100).toFixed(
                          1,
                        )}
                        %
                      </div>
                      <div>
                        蓄力时间 {currentDefenderPanel.charge_time.toFixed(2)}
                      </div>
                    </div>
                  </div>
                )}
            ...
            {battleFinished && (
              <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-base text-emerald-700">
                战斗结束：
                {battleResult.result === "attacker_win"
                  ? `${battleResult.attacker_panel.name} 胜利`
                  : battleResult.result === "defender_win"
                    ? `${battleResult.defender_panel.name} 胜利`
                    : "平局"}
              </div>
            )}
          </div>
