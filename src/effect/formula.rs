...
impl FormulaCalculator {
    ...
    pub fn evaluate_battle(
        ...
        if let Some(ref result) = context.attack_result {
            ctx.var("attack_total_output", result.total_output)
                ...
                .var("attack_broke_qi_defense", if result.broke_qi_defense { 1.0 } else { 0.0 });
        }
    ...
    fn add_panel_to_context(ctx: &mut Context, prefix: &str, panel: &CharacterPanel) {
        ...
        ctx.var(&format!("{}_x", prefix), panel.x())
            ...
            .var(&format!("{}_max_qi_output_rate", prefix), panel.max_qi_output_rate)
            .var(&format!("{}_qi_output_rate", prefix), panel.qi_output_rate)
            .var(&format!("{}_damage_bonus", prefix), panel.damage_bonus)
            .var(&format!("{}_damage_reduction", prefix), panel.damage_reduction)
            .var(&format!("{}_max_damage_reduction", prefix), panel.max_damage_reduction)
            // 武技相关属性
            ...
            .var(&format!("{}_qi_recovery_rate", prefix), panel.qi_recovery_rate)
    ...
    fn evaluate_with_context(formula: &str, ctx: Context) -> Result<f64, String> {
        ...
        let expr = Expr::from_str(&normalized)
            .map_err(|e| format!("公式解析错误: {} (原始公式: '{}', 规范化公式: '{}')", e, formula, normalized))?;
...
mod tests {
    ...
    fn test_battle_formula() {
        ...
        let opponent_panel = CharacterPanel::new("对手".to_string(), ThreeDimensional::new(15, 25, 35));
    ...
    fn test_attack_result_formula() {
        ...
        // 测试使用攻击结果
        assert_eq!(
            FormulaCalculator::evaluate_battle("attack_total_output - attack_total_defense", &context).unwrap(),
            20.0
        );
