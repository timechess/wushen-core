...
impl BattleRecordTemplate {
    ...
    pub fn generate(
        ...
        if let Some(result_data) = battle_result {
            ...
            result = result.replace("{total_output}", &format!("{:.1}", result_data.total_output));
            result = result.replace("{total_defense}", &format!("{:.1}", result_data.total_defense));
            result = result.replace("{reduced_output}", &format!("{:.1}", result_data.reduced_output));
            result = result.replace("{attacker_qi_consumed}", &format!("{:.1}", result_data.attacker_qi_consumed));
            result = result.replace("{defender_qi_consumed}", &format!("{:.1}", result_data.defender_qi_consumed));
            result = result.replace("{broke_qi_defense}", if result_data.broke_qi_defense { "是" } else { "否" });
        }
...
mod tests {
    ...
    fn test_simple_template() {
        let template = BattleRecordTemplate::new("{self_name}的{target}{operation}了{value}".to_string());
    ...
    fn test_template_with_opponent() {
        let template = BattleRecordTemplate::new("{self_name}对{opponent_name}造成了{hp_damage}点伤害".to_string());
        let self_panel = CharacterPanel::new("张三".to_string(), ThreeDimensional::new(10, 8, 12));
        let opponent_panel = CharacterPanel::new("李四".to_string(), ThreeDimensional::new(9, 7, 11));
    ...
    fn test_template_deserialize() {
        ...
        assert_eq!(template.template, "{self_name}的{target}{operation}了{value}");
    }
}
