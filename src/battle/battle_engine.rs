...
use crate::effect::{
    executor::{EntryExecutor, EntryEffect},
    trigger::Trigger,
...
use super::{
    ...
    battle_state::{BattleState, BattleResult, Side},
    battle_calculator::{BattleCalculator, BattleCalculationResult},
...
impl BattleEngine {
    ...
    pub fn new(
        ...
        let action_bar = ActionBar::new(
            side_a_panel.charge_time,
            side_b_panel.charge_time,
        );
    ...
    fn handle_before_attack(&mut self) {
        ...
        if let Some(ref temp) = self.attacker_temp {
            if let Some(ref template) = temp.attack_skill_log_template {
                ...
                let defender_name = self.defender_temp.as_ref().map(|p| p.name.clone()).unwrap_or_default();
    ...
    fn handle_before_defense(&mut self) {
        ...
        if let Some(ref temp) = self.defender_temp {
            if let Some(ref template) = temp.defense_skill_log_template {
                ...
                let attacker_name = self.attacker_temp.as_ref().map(|p| p.name.clone()).unwrap_or_default();
    ...
    fn handle_calculating(&mut self) {
        // 获取临时面板进行战斗计算
        let attacker_temp = self.attacker_temp.as_mut().expect("结算时必须有攻击者临时面板");
        let defender_temp = self.defender_temp.as_mut().expect("结算时必须有防御者临时面板");
    ...
    fn apply_effects(
        ...
        for entry_effect in effects {
            self.apply_single_effect(&entry_effect.effect, source_side, &entry_effect.source_id, battle_result);
        }
    }
    ...
    fn apply_single_effect(
        ...
        match effect {
            ...
            Effect::ModifyPercentage { target, value, operation, target_panel, can_exceed_limit, is_temporary, battle_record_template: _ } => {
                ...
                // 生成战斗记录
                if let Some(description) = self.generate_effect_description(effect, source_side, battle_result) {
                    self.record_with_delta(BattleRecord::EntryTriggered {
    ...
    fn handle_extra_attack(
        ...
        // 获取目标的减伤
        let damage_reduction = if let Some(temp) = self.get_temp_panel_by_side(target_side) {
            temp.damage_reduction.min(temp.max_damage_reduction)
        } else {
            let panel = self.get_panel(target_side);
            panel.damage_reduction.min(panel.max_damage_reduction)
        };
        
        // 计算减伤后伤害
        let reduced_damage = output * (1.0 - damage_reduction);
        
        // 应用伤害到目标
        if let Some(temp) = self.get_temp_panel_mut_by_side(target_side) {
            temp.hp -= reduced_damage;
            temp.clamp_hp();
        } else {
            self.get_panel_mut(target_side).hp -= reduced_damage;
            self.get_panel_mut(target_side).clamp_hp();
        }
    ...
    fn calculate_formula_value(
        ...
        let opponent_panel = Self::battle_panel_to_character_panel(self.get_panel(source_side.opposite()));
    ...
    fn update_record_deltas(
        ...
        let side_a_opt = if side_a_delta.is_empty() { None } else { Some(side_a_delta) };
        let side_b_opt = if side_b_delta.is_empty() { None } else { Some(side_b_delta) };
        ...
        match record {
            ...
            BattleRecord::ActionBarUpdate { side_a_progress, side_b_progress, .. } => {
            ...
            BattleRecord::EntryTriggered { entry_id, description, .. } => {
            ...
            BattleRecord::CalculationResult { attacker_name, attacker_skill, defender_name, defender_skill, result, description, .. } => {
            ...
            BattleRecord::ExtraAttack { source_name, target_name, output, reduced_damage, entry_id, description, .. } => {
