...
impl CharacterPanel {
    ...
    pub fn create_cultivation_context(&self, manual_manager: &crate::cultivation::manual_manager::ManualManager) -> crate::effect::condition::CultivationContext {
        let internal_type = self.current_internal_id.as_ref()
            .and_then(|id| manual_manager.get_internal(id))
            .map(|internal| internal.manual.manual_type.clone());
        
        let attack_skill_type = self.current_attack_skill_id.as_ref()
            .and_then(|id| manual_manager.get_attack_skill(id))
            .map(|skill| skill.manual.manual_type.clone());
        
        let defense_skill_type = self.current_defense_skill_id.as_ref()
